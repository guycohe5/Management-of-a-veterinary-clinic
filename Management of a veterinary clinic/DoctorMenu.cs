    ...
    public partial class DoctorMenu : Form
    {
      ...
      private void InfoshowAnimals(Row row)
        ...
        private void Load_HistoryA(string owner)
        {
            ...
            if (animals_list != null)
            {
                AnimalNameH.Items.Clear();
                foreach (Row row in animals_list)
                    AnimalNameH.Items.Add(row.GetColValue("namea").ToString());
            }
            else
                animals_list = new List<Row>();
        ...
        {
            string idc = IdclientH.Text;
            string idac = IdclientH.Text;
            string namea = AnimalNameH.Text;
            ...
            else
            {
                ...
                string qureyh = SQL_Queries.Select("animals", condition);
                List<Condition> conditions = new List<Condition>() {
                new Condition("idat",idac),new Condition("animal_name",namea)

                };
                string qureyh2 = SQL_Queries.Select("treatment", conditions, "and");
                List<Row> teabl = Access.getObjects(qureyh);
                if (teabl != null)
                {
                    if (teabl.Count == 0)
                    {
        ...
        {
            string idat = IdclientH.Text;
            string animal_name = AnimalNameH.Text;
            ...
            if (teabl != null)
            {
                ...
                else
                {
                    ...
                    Row row = teabl[0];
                    Load_DrugsHistory(row.GetColValue("idclientsdrugs").ToString(), CalendarHistory.SelectionRange.Start.ToShortDateString(), AnimalNameH.Text);
            ...
            if (teablhis != null)
            {
                ...
                else
                {
                    ...
                    Load_VaccinesHistory(row9.GetColValue("idclientsvaccines").ToString(), CalendarHistory.SelectionRange.Start.ToShortDateString(), AnimalNameH.Text);
                }
            }
            else
            {
